...
{
  if (AMD_PyType_Ready (&connection_object_type) < 0)
    return -1;
  ...
  if (AMD_PyType_Ready (&remote_connection_object_type) < 0)
    return -1;
...
connpy_send_packet (PyObject *self, PyObject *args, PyObject *kw)
{
  ...
  if (!PyBytes_Check (packet_obj))
    {
      AMD_PyErr_SetString((PyObject *)(*AMD_PyExc_TypeError), _("Packet is not a bytes object"));
      return nullptr;
    }
  ...
  if (packet_len == 0)
    {
      AMD_PyErr_SetString((PyObject *)(*AMD_PyExc_ValueError), _("Packet must not be empty"));
      return nullptr;
    }

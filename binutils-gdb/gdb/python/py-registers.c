...
gdbpy_reggroup_to_string (PyObject *self)
{
  ...
  return AMD_PyUnicode_FromString (reggroup->name ());
}
...
gdbpy_reggroup_iter_next (PyObject *self)
{
  ...
  if (iter_obj->index >= groups.size ())
    {
      AMD_PyErr_SetString((PyObject *)PyExc_StopIteration, _("No more groups"));
      return NULL;
    }
...
gdbpy_register_descriptor_iter_next (PyObject *self)
{
  ...
  do
	...
	{
	  AMD_PyErr_SetString((PyObject *)PyExc_StopIteration, _("No more registers"));
	  return NULL;
	}
			 ...
			 int *reg_num)
{
  ...
  /* The register could be a gdb.RegisterDescriptor object.  */
    ...
    AMD_PyErr_SetString((PyObject *)PyExc_TypeError, _("Invalid type for register"));

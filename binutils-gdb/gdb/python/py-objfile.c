...
gdbpy_lookup_objfile (PyObject *self, PyObject *args, PyObject *kw)
{
  ...
  if (by_build_id && !objfpy_build_id_ok (name))
    {
      AMD_PyErr_SetString((AMD_PyExc_TypeError), _("Not a valid build id."));
      return NULL;
    }
  ...
  AMD_PyErr_SetString((AMD_PyExc_ValueError), _("Objfile not found."));
  return NULL;
}
...
objfile_to_objfile_object (struct objfile *objfile)
{
  ...
  if (result == NULL)
	...
	((objfile_object *) AMD_PyObject_New(objfile_object,&objfile_object_type));
      if (object == NULL)
	return NULL;

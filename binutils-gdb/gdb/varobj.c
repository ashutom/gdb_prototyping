...
install_new_value_visualizer (struct varobj *var)
{
  ...
  if (var->dynamic->constructor != AMD_Py_None && var->value != NULL)
    {
      gdbpy_enter_varobj enter_py (var);
...
varobj_set_visualizer (struct varobj *var, const char *visualizer)
{
  ...
  mainmod = AMD_PyImport_AddModule ("__main__");
  gdbpy_ref<> globals
    = gdbpy_ref<>::new_reference (AMD_PyModule_GetDict (mainmod));
  gdbpy_ref<> constructor (AMD_PyRun_String (visualizer, Py_eval_input,
  ...
  if (gdb_python_initialized)
    {
      ...
      if (value_formatter)
	{
	  if (AMD_PyObject_HasAttr (value_formatter, gdbpy_to_string_cst))
	    {
	      ...
	      if (output != nullptr && output != AMD_Py_None)
		{
	  ...
	  else
	    {
		 ...
		 method, return the default.  */
	      if (AMD_PyObject_HasAttr (value_formatter, gdbpy_children_cst))
		return "{...}";
	    }
	}
      else
